...
const seedData = async () => {
  ...
  let pharmacyOwners = users.filter((user) => user.role === "pharmacy");

  console.log("[+] Seeding presciptions....");
  let prescriptions = await Prescription.insertMany(
    customers.map((customer) => prescriptionSeed(customer._id))
  );

  console.log("[+] Seeding pharmacies....");
  let generatedPharmacies = pharmacyOwners.map((pharmacy, index) =>
    pharmacySeed(index, pharmacy._id)
  );
  
  let pharmaciesList = await Pharmacy.insertMany(generatedPharmacies);
  ...
  for (let index = 0; index < pharmaciesList.length; index++) {
    ...
    customers.forEach(async (customer) => {
      ...
      let userPrescriptions = prescriptions.filter(
        (p) => p.owner == customer._id
      );

      let order = await orderSeed(
        customer._id,
        pharmacy._id,
        products,
        userPrescriptions[0]
